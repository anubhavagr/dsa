...
def md_inline(s: str) -> str:
    ...
    def link(m):
        ...
        # internal repo link (../cheatsheets/01-x.md, ../README.md, …):
        # keep the href relative — the frontend intercepts it and opens the doc viewer
        label = text
        pm = re.fullmatch(r"(?:\.\./)?([^/]+)\.md", text)
        if pm:  # bare path labels like ../README.md → README
            label = pm.group(1)
        return f'<a href="{url}" class="int">{label}</a>'
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path in ("/", "/index.html"):
            ...
            f = ROOT / rel
            if not f.exists():
                self._send(404, {"error": "no such doc"})
                return
            first = next((l.lstrip("# ").strip() for l in f.read_text(encoding="utf-8").splitlines() if l.startswith("#")), rel)
            self._send(200, {"title": first, "path": rel, "markdown": f.read_text(encoding="utf-8")})
        else:
